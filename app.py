...
# If using PostgreSQL (e.g., AWS RDS), construct the database URL
if not database_url:
...
# For Render, ensure the persistent SQLite path is used if available
if not database_url:
    persistent_db_path = "/var/lib/sqlite/Users.db"
    local_db_path = "instance/Users.db"
    database_url = f"sqlite:///{persistent_db_path if os.path.exists('/var/lib/sqlite') else local_db_path}"
